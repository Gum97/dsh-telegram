  ...
  ctx.effect(async () => {
    ...
    async function onUpdate(update) {
      ...
      if (!allowed(message.from?.id)) {
        await api.sendHtml(message.chat.id, '⛔️ Bạn không có quyền dùng bot này.');
        return;
      }
    ...
    async function handleStart(message) {
      ...
      if (decision.kind === 'denied') {
        await api.sendHtml(
          message.chat.id,
          '⛔️ Bot này đã có chủ. Nhờ chủ bot thêm id của bạn: ' +
            `<code>${escapeHtml(String(message.from?.id ?? '?'))}</code>`,
        );
        return true;
      }
      ...
      try {
        ...
        await api.sendHtml(
          message.chat.id,
          '⚠️ Không lưu được quyền sở hữu. Hãy thêm id của bạn trong Settings: ' +
            `<code>${escapeHtml(decision.userId)}</code>`,
        );
        return true;
      }
      ...
      await api.sendHtml(
        ...
        [
          ...
          '',
          `Từ giờ chỉ id <code>${escapeHtml(decision.userId)}</code> dùng được bot. ` +
